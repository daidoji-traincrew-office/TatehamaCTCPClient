...
namespace TatehamaCTCPClient.Manager
{
    public class CTCPManager {
        ...
        public CTCPManager(PictureBox pictureBox, CTCPWindow window) {
            ...
            // 試験表示
            {
                ...
                foreach (var b in destinationButtons.Values) {
                    lock (syncButtonsImage) {
                        gd.DrawImage(buttonsImage, new Rectangle(b.Location.X, b.Location.Y, b.Type.Size.Width, b.Type.Size.Height), b.Type.Location.X, b.Type.Location.Y + b.Type.Size.Height + 1, b.Type.Size.Width, b.Type.Size.Height, GraphicsUnit.Pixel, new());
                        middleCharSet.DrawText(gd, b.Label, b.Location.X + b.Type.LabelPosition.X, b.Location.Y + b.Type.LabelPosition.Y, b.Type.LabelSize.Width, b.Type.LabelSize.Height, iaDest, usemultiCharacter: false);
                        gi.DrawImage(buttonsImage, new Rectangle(b.Location.X, b.Location.Y, b.Type.Size.Width, b.Type.Size.Height), b.Type.Location.X, b.Type.Location.Y, b.Type.Size.Width, b.Type.Size.Height, GraphicsUnit.Pixel, new());
                        middleCharSet.DrawText(gi, b.Label, b.Location.X + b.Type.LabelPosition.X, b.Location.Y + b.Type.LabelPosition.Y, b.Type.LabelSize.Width, b.Type.LabelSize.Height, iaDest, usemultiCharacter: false);
                    }
                    var p = new PictureBox();
                    window.Panel1.Controls.Add(p);
                    p.Location = hover ? b.Location : new Point(-100, -100);
                ...
                foreach (var w in trainWindows.Values) {
                    ...
                    if (randValue > 0) {
                        ...
                        switch (randValue - 9) {
                            case 1:
                                numFooter = numFooter.Insert(0, "S");
                                break;
                            case 2:
                                numFooter = numFooter.Insert(0, "T");
                                break;
                        }
                        randValue = rand.Next(12);
                        switch (randValue - 8) {
                        ...
                        /*Debug.WriteLine($"{numHeader}{numBody}{numFooter}");*/
                        DrawTrainNumber(gd, numHeader, numBody, numFooter, w.Location.X, w.Location.Y);

                    }
                    else {
                        var l = middleCharSet.MultiCharacters.Values.Where(s => s.Name.Length > 2 && s.Size.Width > 20).ToArray();
        ...
        private void LoadRoutes(string fileName) {
            try {
                ...
                while (line != null) {
                    ...
                    var route = new Route(texts[0], group, track, station, forcedDrop);
        ...
        public void LoadAlertDep(string fileName) {
            try {
                using var sr = new StreamReader($".\\tsv\\{fileName}");
                sr.ReadLine();
                var line = sr.ReadLine();
                while (line != null) {
                    ...
                    if (texts.Length < 3 || texts.Any(t => t.Length <= 0)) {
                        continue;
                    }
                    StationSetting? station = null;
                    if (texts[0].Length > 0) {
                    ...
                    alertDepSettings.Add(texts[0], new(texts[0], station, texts[1], texts[2]));
        ...
        public void UpdateCTCP(bool updatedBlinkStateFast = false, bool updatedBlinkStateSlow = false) {
            ...
            foreach (var b in destinationButtons.Values) {
                ...
                if (b.Lighting == LightingType.LIGHTING || blinkFast && b.Lighting == LightingType.BLINKING_FAST || blinkSlow && b.Lighting == LightingType.BLINKING_SLOW) {
                    ...
                    middleCharSet.DrawText(g, b.Label, b.Location.X + b.Type.LabelPosition.X, b.Location.Y + b.Type.LabelPosition.Y, b.Type.LabelSize.Width, b.Type.LabelSize.Height, iaDest, usemultiCharacter: false);
                }
            }
            ...
            try {
                ...
                if (!ServerCommunication.Error) {
                    ...
                    TaskDialog.ShowDialog(window, new TaskDialogPage {
                        Caption = $"描画{window.ErrorText} | {window.SystemNameLong} - ダイヤ運転会",
                        Heading = $"描画{window.ErrorText}",
                        Icon = TaskDialogIcon.Error,
                        Text = CTCPWindow.IsAprilFool ? "なんか画面のお絵描きをしくじっちゃったみたいね。\nソフト作った人に教えてあげた方がいいんじゃないかしら..." : "CTCP画面の描画に失敗しました。\nCTCP製作者に状況を報告願います。"
                    });
                }
            }
        ...
        public void DrawTrainNumber(Graphics g, string numHeader, string numBody, string numFooter, int x, int y, bool invert = false, bool lightsOut = false, bool fill = false) {
            ...
            if (invert) {
                g.FillRectangle(new SolidBrush(classColor.Value), x, y, 67, 13);
            }
            else if (fill) {
