...
namespace TatehamaCTCPClient.Manager {
    public class TrainAlertManager {
        ...
        public static bool RemoveAlert(string routeGroup, string trainNumber, AlertType? type = null) {
            var l = new List<TrainAlert>(trainAlerts);
            var v = false;
            var i = true;
            foreach (var a in l) {
                if (a.TrainNumber == trainNumber && a.RouteGroup == routeGroup && (type == null || type == a.Type)) {
                    v |= trainAlerts.Remove(a);
                }
                else if (a.Important) {
                    i = false;
