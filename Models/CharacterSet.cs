...
namespace TatehamaCTCPClient.Models {
    public class CharacterSet {
        ...
        public bool DrawText(Graphics g, string text, int x, int y, int width, int height, ImageAttributes ia, ContentAlignment align = ContentAlignment.MiddleCenter, bool usemultiCharacter = true) {
            if(text.Length <= 0) {
                return true;
            }
            if (usemultiCharacter && multiCharacters.TryGetValue(text, out var mc)) {
