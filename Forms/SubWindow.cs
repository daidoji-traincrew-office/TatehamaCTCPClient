    ...
    public partial class SubWindow : Form {
        ...
        public SubWindow(Point location, Size size, CTCPManager displayManager, string? name = null) {
            ...
            foreach (var b in buttons) {
                ...
                : (sender, e) => {
                    if (displayManager.Started) {
                        displayManager.Window.ReservedUpdate = b.OnClick() ? 2 : 0;
                    }
                };
        ...
        private void labelTrainAlert_Click(object sender, EventArgs e) {
            displayManager.Window.OpenNavigationWindow();
        ...
        private void menuItemErrorLog_Click(object sender, EventArgs e) {
            ...
            var result = TaskDialog.ShowDialog(this, new TaskDialogPage {
                Caption = $"ログファイル出力確認 | {displayManager.Window.SystemNameLong} - ダイヤ運転会",
                Heading = $"ログファイル出力確認",
                Icon = TaskDialogIcon.Information,
                Text = CTCPWindow.IsAprilFool ? $"{displayManager.Window.ErrorText}ログファイル、本当に出力しちゃっていいかしら？" : $"{displayManager.Window.ErrorText}ログファイルを出力しますか？",
            ...
            if (result == TaskDialogButton.Yes) {
                ...
                TaskDialog.ShowDialog(this, new TaskDialogPage {
                    Caption = $"{displayManager.Window.ErrorText}ログ出力 | {displayManager.Window.SystemNameLong} - ダイヤ運転会",
                    Heading = $"{displayManager.Window.ErrorText}ログ出力",
                    Icon = TaskDialogIcon.Information,
                    Text = CTCPWindow.IsAprilFool ? $"{displayManager.Window.ErrorText}ログをErrorLog.txtとして出力したわ！\n次回起動した時に消えるかもだからしっかりとっとくのよ！" :
                        $"{displayManager.Window.ErrorText}ログがErrorLog.txtとして出力されました。\n（ErrorLog.txtは次回起動後に削除される場合があります）"
                });
            }
            OpeningDialog = false;
        }
