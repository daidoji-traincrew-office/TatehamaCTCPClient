...
namespace TatehamaCTCPClient.Forms {
    public partial class NavigationWindow : Form {
        ...
        public NavigationWindow(CTCPManager displayManager) {
            ...
            UpdateNotification();
            UpdateAlert();
            trackBarVolumeMaster.Value = CTCPWindow.VolumeMaster;
            buttonMuteMaster.Text = CTCPWindow.MuteMaster ? "ﾐｭｰﾄ解除" : "ミュート";
            labelVolumeMaster.ForeColor = CTCPWindow.MuteMaster ? Color.Gray : Color.Black;
            trackBarVolumeWarning.Value = CTCPWindow.VolumeWarning;
            buttonMuteWarning.Text = CTCPWindow.MuteWarningSound ? "ﾐｭｰﾄ解除" : "ミュート";
            labelVolumeWarning.ForeColor = CTCPWindow.MuteWarningSound ? Color.Gray : Color.Black;
            trackBarVolumeSpawn.Value = CTCPWindow.VolumeSpawn;
            buttonMuteSpawn.Text = CTCPWindow.MuteSpawnSound ? "ﾐｭｰﾄ解除" : "ミュート";
            labelVolumeSpawn.ForeColor = CTCPWindow.MuteSpawnSound ? Color.Gray : Color.Black;
            trackBarVolumeAprDown.Value = CTCPWindow.VolumeAprDown;
            buttonMuteAprDown.Text = CTCPWindow.MuteAprDownSound ? "ﾐｭｰﾄ解除" : "ミュート";
            labelVolumeAprDown.ForeColor = CTCPWindow.MuteAprDownSound ? Color.Gray : Color.Black;
            trackBarVolumeAprUp.Value = CTCPWindow.VolumeAprUp;
            buttonMuteAprUp.Text = CTCPWindow.MuteAprUpSound ? "ﾐｭｰﾄ解除" : "ミュート";
            labelVolumeAprUp.ForeColor = CTCPWindow.MuteAprUpSound ? Color.Gray : Color.Black;
            trackBarVolumeAprPi.Value = CTCPWindow.VolumeAprPi;
            buttonMuteAprPi.Text = CTCPWindow.MuteAprPiSound ? "ﾐｭｰﾄ解除" : "ミュート";
            labelVolumeAprPi.ForeColor = CTCPWindow.MuteAprPiSound ? Color.Gray : Color.Black;

            labelVolumeWarning.Text = $"警告・{displayManager.Window.ErrorText}音";

            if (CTCPWindow.IsAprilFool) {
                label1.Text = "このソフトで信号扱いする管轄駅を選択してちょうだい。\r\n駅名部分をクリックすると範囲選択できるわよ！（ダブルクリックで単駅選択）\r\n！ここから信号扱いしない駅はすぐに管轄駅から外すのよ！";
            }
        ...
        private void PlaceStations() {
            ...
            for (var i = 0; i < stations.Count; i++) {
                ...
                c.CheckedChanged += (sender, e) => {
                    ...
                    displayManager.Window.UpdateLabelTrainAlert();
                    p.BackColor = s.Active ? Color.LightBlue : SystemColors.ControlLight;
        ...
        private void trackBarVolumeMaster_ValueChanged(object sender, EventArgs e) {
            ...
            CTCPWindow.VolumeMaster = trackBarVolumeMaster.Value;
        }
        ...
        private void buttonMuteMaster_Click(object sender, EventArgs e) {
            CTCPWindow.MuteMaster = !CTCPWindow.MuteMaster;
            buttonMuteMaster.Text = CTCPWindow.MuteMaster ? "ﾐｭｰﾄ解除" : "ミュート";
            labelVolumeMaster.ForeColor = CTCPWindow.MuteMaster ? Color.Gray : Color.Black;
        }
        ...
        private void trackBarVolumeWarning_ValueChanged(object sender, EventArgs e) {
            ...
            CTCPWindow.VolumeWarning = trackBarVolumeWarning.Value;
        }
        ...
        private void buttonMuteWarning_Click(object sender, EventArgs e) {
            CTCPWindow.MuteWarningSound = !CTCPWindow.MuteWarningSound;
            buttonMuteWarning.Text = CTCPWindow.MuteWarningSound ? "ﾐｭｰﾄ解除" : "ミュート";
            labelVolumeWarning.ForeColor = CTCPWindow.MuteWarningSound ? Color.Gray : Color.Black;
        }

        private void trackBarVolumeSpawn_ValueChanged(object sender, EventArgs e) {
            toolTip1.SetToolTip(trackBarVolumeSpawn, trackBarVolumeSpawn.Value.ToString());
            CTCPWindow.VolumeSpawn = trackBarVolumeSpawn.Value;
        }

        private void trackBarVolumeSpawn_MouseUp(object sender, MouseEventArgs e) {
            CTCPWindow.PlaySpawnSound();
        }

        private void buttonMuteSpawn_Click(object sender, EventArgs e) {
            CTCPWindow.MuteSpawnSound = !CTCPWindow.MuteSpawnSound;
            buttonMuteSpawn.Text = CTCPWindow.MuteSpawnSound ? "ﾐｭｰﾄ解除" : "ミュート";
            labelVolumeSpawn.ForeColor = CTCPWindow.MuteSpawnSound ? Color.Gray : Color.Black;
        }

        private void trackBarVolumeAprDown_ValueChanged(object sender, EventArgs e) {
            toolTip1.SetToolTip(trackBarVolumeAprDown, trackBarVolumeAprDown.Value.ToString());
            CTCPWindow.VolumeAprDown = trackBarVolumeAprDown.Value;
        }

        private void trackBarVolumeAprDown_MouseUp(object sender, MouseEventArgs e) {
            CTCPWindow.PlayAprDownSound();
        }

        private void buttonMuteAprDown_Click(object sender, EventArgs e) {
            CTCPWindow.MuteAprDownSound = !CTCPWindow.MuteAprDownSound;
            buttonMuteAprDown.Text = CTCPWindow.MuteAprDownSound ? "ﾐｭｰﾄ解除" : "ミュート";
            labelVolumeAprDown.ForeColor = CTCPWindow.MuteAprDownSound ? Color.Gray : Color.Black;
        }

        private void trackBarVolumeAprUp_ValueChanged(object sender, EventArgs e) {
            toolTip1.SetToolTip(trackBarVolumeAprUp, trackBarVolumeAprUp.Value.ToString());
            CTCPWindow.VolumeAprUp = trackBarVolumeAprUp.Value;
        }

        private void trackBarVolumeAprUp_MouseUp(object sender, MouseEventArgs e) {
            CTCPWindow.PlayAprUpSound();
        }

        private void buttonMuteAprUp_Click(object sender, EventArgs e) {
            CTCPWindow.MuteAprUpSound = !CTCPWindow.MuteAprUpSound;
            buttonMuteAprUp.Text = CTCPWindow.MuteAprUpSound ? "ﾐｭｰﾄ解除" : "ミュート";
            labelVolumeAprUp.ForeColor = CTCPWindow.MuteAprUpSound ? Color.Gray : Color.Black;
        }

        private void trackBarVolumeAprPi_ValueChanged(object sender, EventArgs e) {
            toolTip1.SetToolTip(trackBarVolumeAprPi, trackBarVolumeAprPi.Value.ToString());
            CTCPWindow.VolumeAprPi = trackBarVolumeAprPi.Value;
        }

        private void trackBarVolumeAprPi_MouseUp(object sender, MouseEventArgs e) {
            CTCPWindow.PlayAprPiSound();
        }

        private void buttonMuteAprPi_Click(object sender, EventArgs e) {
            CTCPWindow.MuteAprPiSound = !CTCPWindow.MuteAprPiSound;
            buttonMuteAprPi.Text = CTCPWindow.MuteAprPiSound ? "ﾐｭｰﾄ解除" : "ミュート";
            labelVolumeAprPi.ForeColor = CTCPWindow.MuteAprPiSound ? Color.Gray : Color.Black;
        }
